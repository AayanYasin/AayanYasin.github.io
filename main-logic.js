...
function openGallery(cat) {
    ...
    document.getElementById('galleryTag').innerHTML = data.tags.map(t => `<span class="tag-pill">${t}</span>`).join('');
    
    // Pass items and the category key
...
function openProjectPage(category, itemIndex) {
    ...
    const categoryTitle = projectData[category].title;
    
    // Fill Text Elements
    document.getElementById('pageTitle').innerText = project.title;
    document.getElementById('pageTag').innerText = categoryTitle;
